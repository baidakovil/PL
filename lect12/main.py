...
def cmd_info(message):
    bot.send_message(message.chat.id, "Info method is used to show you what I am capable of.\n"
                                      ...
                                      "I only have info for today, yesterday and the day before yesterday\n"
...
def cmd_reset(message):
    ...
    dbworker.set_current_state(message.chat.id, config.States.S_ENTER_DAY.value)
...
def cmd_start(message):
    ...
    bot.send_message(message.chat.id, "Greetings again! I'm coronabot :) \n"
                                      "You gotta specify which day's statistics you want to get: /today, /yesterday, /beforeyesterday.\n"
...
def cmd_day(message):
    ...
    if message.text.lower().strip() == '/yesterday':
        ...
        dbworker.set_current_state(message.chat.id, config.States.S_COUNTRY_OR_REGION.value)
        ...
        dbworker.set_current_state(message.chat.id, config.States.S_COUNTRY_OR_REGION.value)
    elif message.text.lower().strip() == '/beforeyesterday':
        # day = 0
        bot.send_message(message.chat.id, "Ok, we've specified the date for statistics. It's time to go further. \n"
                                          "Do you want to know things about /regions or /countries?\n"
                                          "You could also type /info to know more about me.\n"
        ...
        dbworker.set_property(str(message.chat.id) + 'day', 'beforeyesterday')  # запишем день в базу
        dbworker.set_current_state(message.chat.id, config.States.S_COUNTRY_OR_REGION.value)
    else:
...
def cmd_country_or_region(message):
    ...
    if message.text.lower().strip() == '/countries':
        ...
        dbworker.set_current_state(message.chat.id, config.States.S_ENTER_COUNTRY_OR_REGION.value)
...
def cmd_list_countries(message):
    ...
    if country.strip() == 'regions':
        ...
        lst = [i for i in list(x[8:232]) if i!='']
    ...
    if len(errors) == 0:
        if countries != list():
            ...
            dbworker.set_current_state(message.chat.id, config.States.S_ENTER_FIELDS_LIST.value)
        else:
...
def cmd_list_fields(message):
    ...
    if errors == []:
        if fields != []:
            ...
            dbworker.del_state(str(message.chat.id) + 'country')
            dbworker.del_state(str(message.chat.id) + 'list_countries')
            bot.send_message(message.chat.id, tabulate(for_sending, headers=for_sending.columns, tablefmt="unsafehtml"))
        else:
